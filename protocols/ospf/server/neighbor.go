...
type neighbor struct {
	...
	state    NeighborState
	primary  bool
	ddSeqNum *uint32 // atomic counter
...
func (n *neighbor) monitorEvents(ctx context.Context) {
	for {
		...
		n.state = n.runStateMachine(ctx, event)
	}
}
...
func (n *neighbor) runStateMachine(ctx context.Context, event NeighborEvent) NeighborState {
	...
	case matchN(NbrStateInit, NbrEvent2WayReceived):
		...
		seqNum := atomic.LoadUint32(n.ddSeqNum)
		if seqNum == 0 {
			atomic.StoreUint32(n.ddSeqNum, uint32(time.Now().Unix()))
		} else {
			atomic.AddUint32(n.ddSeqNum, 1)
		}

		flags := packet.DBFlagInit & packet.DBFlagMore & packet.DBFlagMS
		n.ifMgmt.sendDatabaseDescription(n, packet.DatabaseDescription{
			DDSequenceNumber: atomic.LoadUint32(n.ddSeqNum),
			DBFlags:          flags,
		})
		go func() {
			ticker := time.NewTicker(n.ifMgmt.GetConfig().RxmtInterval)
			for {
				select {
				case <-ctx.Done():
					return
				case <-ticker.C:
					seqNum := atomic.AddUint32(n.ddSeqNum, 1)
					n.ifMgmt.sendDatabaseDescription(n, packet.DatabaseDescription{
						DDSequenceNumber: seqNum,
						DBFlags:          flags,
					})
				}
			}
